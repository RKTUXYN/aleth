...
void printHelp()
{
    ...
    cout << setw(30) << "--diskdb" << setw(25) << "Use a disk-backed block and state database for the consensus tests\n";
...
Options::Options(int argc, const char** argv)
{
    ...
    for (auto i = 0; i < argc; ++i)
    {
        ...
        {
            // The blockchain and state tests will use a disk-backed database rather than the in-memory
            // database for storing block and state data. This option is useful so that these
            // integration-style tests can exercise the same code that users hit when they run the client
            // software.
